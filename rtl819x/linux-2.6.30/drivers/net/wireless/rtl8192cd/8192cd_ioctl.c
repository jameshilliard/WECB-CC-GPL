	...
	while (plist != phead) {
		...
		if (((OPMODE & WIFI_AP_STATE) && !memcmp(pstat->hwaddr, mac2addr+6 , 6))
			...
			if (found) {
				...
				if (!add) {
#if !defined(AEI_WECB)
                    // When kernel receives IGMP packet, mcast mac is added or deleted on destinated wlan client.
                    // If mcast mac address is deleted more than once, it steps here.
                    // It is not function error and the printed msg would confuses user. So stop print it any more.

...
./configure --prefix=$CROSS --host=mips-linux  --enable-shared CFLAGS=-Os host_alias=mips-linux CC=mips-linux-gcc LD=mips-linux-ld RANLIB=mips-linux-ranlib --without-python --without-iconv --with-tree --with-writer --with-xpath --with-reader
make
if [ $? -ne 0 ]; then

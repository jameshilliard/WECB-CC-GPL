...
void actAdvsyslog(request *wp, char *path, char *query)
{
	char *strVal = NULL;
	char *mypage = NULL;	

	char syslog_server[BUF_LEN20]={0};
	char syslog_enable[BUF_LEN10]={0};
	...
	{
		strVal = req_get_cstream_var(wp, ("syslog_server"), "");
		//fprintf(stdout,"\n#@#@applyserver=%s\n",strVal);

		if(strVal[0]) {
			kinfo("syslog_server: %s", strVal);

	            if(tr69SetUnfreshLeafDataString(DEVICE_ACTIONTEC_SYSLOG, 0, "ServerIP", strVal))
	            {
	                strcpy(error_buf, "Set Device.X_ACTIONTEC_COM_SYSLOG.ServerIP error!");
			  goto setErr_adv;
	            }
		}

		strVal = req_get_cstream_var(wp, ("syslog_enable"), "");
		//fprintf(stdout,"\n#@#@syslog_enable=%s,d=%d\n",strVal,atoi(strVal));
		if(strVal[0]) {
			kinfo("subnet: %s", strVal);

	            if(tr69SetUnfreshLeafDataInt(DEVICE_ACTIONTEC_SYSLOG, 0, "Enable", atoi(strVal)))
	            {
	                strcpy(error_buf, "Set Device.X_ACTIONTEC_COM_SYSLOG.Enable error!");
			  goto setErr_adv;
	            }
		}
	...
	{	 
	        if(tr69GetUnfreshLeafData(syslog_server, DEVICE_ACTIONTEC_SYSLOG, "ServerIP"))
	        {
	            strcpy(error_buf, "Get Device.X_ACTIONTEC_COM_SYSLOG.ServerIP error!");
	            goto getErr_adv;
	        }
		if(tr69GetUnfreshLeafData(syslog_enable, DEVICE_ACTIONTEC_SYSLOG, "Enable"))
	        {
	            strcpy(error_buf, "Get Device.X_ACTIONTEC_COM_SYSLOG.ServerIP error!");
	            goto getErr_adv;
	        } 
		changeString(syslog_server);
		changeString(syslog_enable);
		//fprintf(stdout,"\n#@#@server=%s,enable=%s\n",syslog_server,syslog_enable);

		 req_format_write(wp, ("%s|%s||"),syslog_server,syslog_enable);			
	}
...
void actAdvTimeZone(request *wp, char *path, char *query)
{
	char *strVal = NULL;
	...
	strVal = req_get_cstream_var(wp, ("command"), "");
       ...
       if(!safe_strcmp(strVal, "apply"))
	{
		strVal = req_get_cstream_var(wp, ("time_zone_value"), "");
		//fprintf(stdout,"\n#@#@time_zone_value=%s\n",strVal);
		if(strVal[0]) {
	            if(tr69SetLeafDataString(DEVICE_TIME, 0, "LocalTimeZone", strVal))
	            {
	                strcpy(error_buf, "Set Device.Time.LocalTimeZone error!");
	            }
		}

		strVal = req_get_cstream_var(wp, ("day_light_value"), "");
		//fprintf(stdout,"\n#@#@day_light_value=%s,d=%d\n",strVal,atoi(strVal));
		if(strVal[0]) {
	            if(tr69SetLeafDataInt(DEVICE_TIME, 0, "X_ACTIONTEC_COM_DaylightSavingsUsed", atoi(strVal)))
	            {
	                strcpy(error_buf, "Set Device.Time.X_ACTIONTEC_COM_DaylightSavingsUsed error!");
	            }
		}
...
void actgetTimeZoneParams(char *parambuf)
{
	char buf[BUF_LEN256] = {0};
	char cur_time[BUF_LEN64] = {0};
 	char local_timezone[BUF_LEN64]={0};
	char day_light_saving[BUF_LEN64]={0};
	
		 
	 if(tr69GetFreshLeafData(cur_time,DEVICE_TIME,"CurrentLocalTime"))
	 {
	      strcpy(error_buf, "Get Device.Time.CurrentLocalTime error!");
	 }

	 if(tr69GetFreshLeafData(local_timezone,DEVICE_TIME,"LocalTimeZone"))
	 {
	      strcpy(error_buf, "Get Device.Time.LocalTimeZone error!");
	 }

	 if(tr69GetFreshLeafData(day_light_saving,DEVICE_TIME,"X_ACTIONTEC_COM_DaylightSavingsUsed"))
	 {
	      strcpy(error_buf, "Get Device.Time.X_ACTIONTEC_COM_DaylightSavingsUsed error!");
	 }
			 
	changeString(cur_time);
	changeString(local_timezone);
	changeString(day_light_saving);


	//fprintf(stdout,"\n#@#@cur_time=%s,timezone=%s,day_light=%d\n",cur_time,local_timezone,day_light_saving);
	snprintf(buf,sizeof(buf), "%s|%s|%s|",cur_time,local_timezone,day_light_saving);
	strcpy(parambuf,buf);
	return;
getErr_adv:
       kerror("error_buf: %s", error_buf);
		 ...
		 if(tr69GetUnfreshLeafData(tr69_user_enable, mdmoid_user_object, "Enable"))
			...
			if(!safe_strcmp(login_username, tr69_username)&&(atoi(tr69_user_enable)!=0))
			{
				...
				if(wp->session_id != 0)
				{
					...
					if(hs != NULL)
					{
						get_bootTime_for_timeout(&start_tm);
						hs->start_time = start_tm;

...
int getCurrentListWac(char *entry2mac,char *entry5mac,char *entry2cmt,char *entry5cmt,int both)
{
	int entryNum = 0;
	MACFILTER_T entry;
	char buf[BUF_LEN100] = {0};
	char tmp_macbuf[BUF_LEN100] = {0};
	char tmp_cmtbuf[BUF_LEN100] = {0};
	int i = 0;
	int entry2Num = 0;
	int entry5Num = 0;
	...
	if(!both)
	{
		if (!apmib_get(MIB_WLAN_MACAC_NUM, (void *)&entryNum)) {				
			strcpy(error_buf, ("Get entry number error!"));
			goto setErr_wac;
		}		
	}
	else
	{
		...
		if (!apmib_get(MIB_WLAN_MACAC_NUM, (void *)&entry2Num)) {				
			strcpy(error_buf, ("Get entry number2 error!"));
			goto setErr_wac;
		}

		wlan_idx = 0;
		if (!apmib_get(MIB_WLAN_MACAC_NUM, (void *)&entry5Num)) {				
			strcpy(error_buf, ("Get entry number5 error!"));
			goto setErr_wac;
		}
	}
		...
		for (i = 1; i <= entryNum; i++) {
			*((char *)&entry) = (char)i;
			if (!apmib_get(MIB_WLAN_MACAC_ADDR, (void *)&entry)) {
				strcpy(error_buf, ("Get MIB_WLAN_MACAC_ADDR error!"));
				goto setErr_wac;
			}
            
			//fprintf(stdout,"entry.comment: %s\n", entry.comment);
			
			changeString(entry.comment);
		
			snprintf(tmp_macbuf, BUF_LEN100, ("%02x:%02x:%02x:%02x:%02x:%02x "),
				entry.macAddr[0], entry.macAddr[1], entry.macAddr[2],
				entry.macAddr[3], entry.macAddr[4], entry.macAddr[5]);

			snprintf(tmp_cmtbuf,BUF_LEN100, ("%s;"),entry.comment);
			
			strcat(entry2mac,tmp_macbuf);
			strcat(entry2cmt,tmp_cmtbuf);
	...
	else 
	{
		...
		for (i = 1; i <= entry2Num; i++) {
			*((char *)&entry) = (char)i;
			if (!apmib_get(MIB_WLAN_MACAC_ADDR, (void *)&entry)) {
				strcpy(error_buf, ("Get MIB_WLAN_MACAC_ADDR2 error!"));
				goto setErr_wac;
			}
		...
		for (i = 1; i <= entry5Num; i++) {
			*((char *)&entry) = (char)i;
			if (!apmib_get(MIB_WLAN_MACAC_ADDR, (void *)&entry)) {
				strcpy(error_buf, ("Get MIB_WLAN_MACAC_ADDR5 error!"));
				goto setErr_wac;
			}
			...
			strcat(entry5mac,tmp_macbuf);
			strcat(entry5cmt,tmp_cmtbuf);
		}
	}
...
int applyWac(request *wp, char *tmpBuf)
{
	...
	char *token = NULL;
	char mac_address[BUF_LEN16] = {0};
	char mac_apply[BUF_LEN20] = {0};
	char cmt_apply[COMMENT_LEN]={0};
	char tmp_entry2mac[BUF_LEN1024]={0};
	char tmp_entry5mac[BUF_LEN1024]={0};
	char tmp_entry2cmt[BUF_LEN1024]={0};
	char tmp_entry5cmt[BUF_LEN1024]={0};
	char entry2mac[BUF_LEN1024]={0};
	char entry5mac[BUF_LEN1024]={0};
	char entry2cmt[BUF_LEN1024]={0};
	char entry5cmt[BUF_LEN1024]={0};
	int enabled = 0;
	int cur_enabled=0;
	int cur_policy=0;
	int entryNum = 0;
	...
	if(strVal[0])
	{
		...
		if(tr69_id != 0)
		{
			...
			{
				cur_enabled=0;
				if(tr69SetUnfreshLeafDataInt("Device.WiFi.AccessPoint", tr69_id, "ACL.X_ACTIONTEC_COM_ACL_Enabled", cur_enabled))
				{
					sprintf(tmpBuf, ("Set Device.WiFi.AccessPoint.%d.X_ACTIONTEC_COM_ACL_Enabled error!"), tr69_id);
				}
		       }
			else
			{
				...
				if(tr69SetUnfreshLeafDataInt("Device.WiFi.AccessPoint", tr69_id, "ACL.X_ACTIONTEC_COM_ACL_Enabled", cur_enabled))
				{
					sprintf(tmpBuf, ("Set Device.WiFi.AccessPoint.%d.ACL.X_ACTIONTEC_COM_ACL_Enabled error!"),tr69_id);
				}
				if(enabled==2)
				{
					cur_policy=1;
					if(tr69SetUnfreshLeafDataInt("Device.WiFi.AccessPoint", tr69_id, "ACL.X_ACTIONTEC_COM_ACL_Deny_Policy", cur_policy))
					{
					sprintf(tmpBuf, ("Set Device.WiFi.AccessPoint.%d.ACL.X_ACTIONTEC_COM_ACL_Deny_Policy error!"), tr69_id);
					kerror("error_buf: %s", tmpBuf);
					}
				}
		       }
		}
	...
	strncpy(mac_apply,strVal,sizeof(mac_apply));
	if(strVal[0])
	{
		...
		strncpy(cmt_apply,strVal,COMMENT_LEN);
		if ( strVal[0] ) {
		...
		if(getCurrentListWac(entry2mac, entry5mac, entry2cmt, entry5cmt, both_freq)<0)
		{
			sprintf(tmpBuf, "getCurrentListWac error!");
			goto setErr_wac;
		}
		if(both_freq==1)
		{

			//fprintf(stdout,"\n***@@@***%d:entry2mac=%s\n,entry2cmt=%s\n,entry5mac=%s\n,entry5cmt=%s\nenbled=%d\n",__LINE__,entry2mac,entry2cmt,entry5mac,entry5cmt,enabled);
			if(vwlan_idx > 0)			
			{
				tr69_id = vwlan_idx;
			}
			...
			if(tr69_id != 0)
			{
				if(tr69SetUnfreshLeafDataString("Device.WiFi.AccessPoint", tr69_id, "ACL.X_ACTIONTEC_COM_ACL_MACList", entry2mac))
				{
					sprintf(tmpBuf, ("Set Device.WiFi.AccessPoint.%d.ACL.X_ACTIONTEC_COM_ACL_MACList error!"), tr69_id);
					kerror("error_buf: %s", tmpBuf);
				}
				if(tr69SetUnfreshLeafDataString("Device.WiFi.AccessPoint", tr69_id, "ACL.X_ACTIONTEC_COM_ACL_MAC_Info", entry2cmt))
				{
					sprintf(tmpBuf, ("Set Device.WiFi.AccessPoint.%d.ACL.X_ACTIONTEC_COM_ACL_MAC_Info error!"), tr69_id);
					kerror("error_buf: %s", tmpBuf);
				}
				if(enabled==2)
				{
			...
			if(tr69_id != 0)
			{
				if(tr69SetUnfreshLeafDataString("Device.WiFi.AccessPoint", tr69_id, "ACL.X_ACTIONTEC_COM_ACL_MACList", entry5mac))
				{
					sprintf(tmpBuf, ("Set Device.WiFi.AccessPoint.%d.ACL.X_ACTIONTEC_COM_ACL_MACList error!"), tr69_id);
					kerror("error_buf: %s", tmpBuf);
				}
				if(tr69SetUnfreshLeafDataString("Device.WiFi.AccessPoint", tr69_id, "ACL.X_ACTIONTEC_COM_ACL_MAC_Info", entry5cmt))
				{
					sprintf(tmpBuf, ("Set Device.WiFi.AccessPoint.%d.ACL.X_ACTIONTEC_COM_ACL_MAC_Info error!"), tr69_id);
					kerror("error_buf: %s", tmpBuf);
				}
		...
		{
			//snprintf(entry2mac,BUF_LEN1024,"%s%s ",tmp_entry2mac,mac_apply);
			//snprintf(entry2cmt,BUF_LEN1024,"%s%s;",tmp_entry2cmt,cmt_apply);
			//fprintf(stdout,"***@@@***%d:entry2mac=%s\n,entry2cmt=%s\n,entry5mac=%s\n,entry5cmt=%s\nenabled=%d\ntr69_id=%d\n",__LINE__,entry2mac,entry2cmt,entry5mac,entry5cmt,enabled,tr69_id);
	...
	if(!both)
	{
		for (i = 1; i <= entryNum; i++) {
			*((char *)&entry) = (char)i;
			...
			snprintf(buf, 100, ("%02x:%02x:%02x:%02x:%02x:%02x|%s|0|%d|%d|%d|%d||"),
				entry.macAddr[0], entry.macAddr[1], entry.macAddr[2],
				entry.macAddr[3], entry.macAddr[4], entry.macAddr[5], entry.comment, i, curr_ssid, enabled,wlan_idx);
	...
	else 
	{
		...
		for (i = 1; i <= entry2Num; i++) {
			*((char *)&entry) = (char)i;
			...
			snprintf(buf, 100, ("%02x:%02x:%02x:%02x:%02x:%02x|%s|10|%d|%d|%d|%d||"),
				entry.macAddr[0], entry.macAddr[1], entry.macAddr[2],
				entry.macAddr[3], entry.macAddr[4], entry.macAddr[5], entry.comment, i, curr_ssid, enabled,wlan_idx);
		...
		for (i = 1; i <= entry5Num; i++) {
			*((char *)&entry) = (char)i;
			...
			snprintf(buf, 100, ("%02x:%02x:%02x:%02x:%02x:%02x|%s|11|%d|%d|%d|%d||"),
				entry.macAddr[0], entry.macAddr[1], entry.macAddr[2],
				entry.macAddr[3], entry.macAddr[4], entry.macAddr[5], entry.comment, i, curr_ssid, enabled,wlan_idx);
...
#define RADIO_IDX_2G 1
#define RADIO_IDX_5G 2
int get_radio_enable_from_dc(int radio_idx)
{
    char radio_obj_path[64] = {0};
    char radio_enable_buf[8] = {0};
    int enable = 0;

    if(radio_idx != RADIO_IDX_2G && radio_idx != RADIO_IDX_5G)
        return 0;

    snprintf(radio_obj_path, sizeof(radio_obj_path), "Device.WiFi.Radio.%d", radio_idx);
    if(!tr69GetUnfreshLeafData(radio_enable_buf, radio_obj_path, "Enable"))
    {
        enable = atoi(radio_enable_buf);
    }else
    {
        enable = 0;
...
void actWlwfps(request *wp, char *path, char *query)
{
	...
	int tr69_id = 0; 
    int radio_enable_2g = 0;
    int radio_enable_5g = 0;
	...
	if(!safe_strcmp(action, "apply"))
	{
		...
		if(strVal[0])
		{
			...
			if(wlan_idx==1)
			{
				for(tr69_id=1;tr69_id<=4;tr69_id++)	
				{
					if(tr69SetUnfreshLeafDataInt("Device.WiFi.AccessPoint", tr69_id, "WPS.Enable", enabled))
					{
						sprintf(error_buf, ("Set Device.WiFi.AccessPoint.%d.WPS.Enable error!"), tr69_id);
						kerror("error_buf: %s", error_buf);
					}
				}
			}
			if(wlan_idx==0)
			{
				for(tr69_id=5;tr69_id<=8;tr69_id++)	
				{
					if(tr69SetUnfreshLeafDataInt("Device.WiFi.AccessPoint", tr69_id, "WPS.Enable", enabled))
					{
						sprintf(error_buf, ("Set Device.WiFi.AccessPoint.%d.WPS.Enable error!"), tr69_id);
		...
		if(strVal[0])
		{
			...
			//fprintf(stdout,"\n###wsc_pin=%d\n",atoi(wsc_pin));
			for(tr69_id=1;tr69_id<=8;tr69_id++)	
			{
				if(tr69SetUnfreshLeafDataInt("Device.WiFi.AccessPoint", tr69_id, "WPS.X_ACTIONTEC_COM_WPS_AP_PIN", atoi(wsc_pin)))
				{
					sprintf(error_buf, ("Set Device.WiFi.AccessPoint.%d.WPS.X_ACTIONTEC_COM_WPS_AP_PIN error!"), tr69_id);
					kerror("error_buf: %s", error_buf);
				}
			}
	...
	{	
        radio_enable_2g = get_radio_enable_from_dc(RADIO_IDX_2G);
        radio_enable_5g = get_radio_enable_from_dc(RADIO_IDX_5G);
		...
		if (disabled) {
			...
			//fprintf(stdout,"\n###both dis,disabled=%d,otherdis=%d\n",disabled,other_disabled);
				enabled=1;
				if(wlan_idx==1)
				{
					for(tr69_id=1;tr69_id<=4;tr69_id++)	
			...
			if (other_disabled) {
				...
				kinfo("both enabled");
                if(!radio_enable_2g && current_wlanidx == 0)
                    sprintf(wps_buf, "%s -sig_pbc wlan0-va0", _WSC_DAEMON_PROG);
                else if(!radio_enable_5g && current_wlanidx == 1)
                    sprintf(wps_buf, "%s -sig_pbc wlan1-va0", _WSC_DAEMON_PROG);
                else
				    sprintf(wps_buf, "%s -sig_pbc wlan0-va0-wlan1-va0", _WSC_DAEMON_PROG);
				}
...
*/
	...
	{			
		...
		convert_hex_to_ascii((unsigned long)num, wsc_pin);		
        	...
        	char cur_if[32];
		char return_pin_buf[BUF_LEN100];
...
#endif
		snprintf(return_pin_buf,sizeof(return_pin_buf),"%s_generate_pin",wsc_pin);
		req_format_write(wp, ("%s"), return_pin_buf);			
#endif			
		return;
	}
...
void actWlgetPinStatus(request *wp, char *path, char *query)
{
	...
	snprintf(buf,sizeof(buf),"%d_disable_appin",getVal);
	req_format_write(wp, ("%s"), buf);
	return;
...
void actWladv(request *wp, char *path, char *query)
{
	...
	if(!safe_strcmp(action, "apply"))
	{
		...
		if(strVal[0])
		{
			...
			if(tr69_id != 0)
			{
				//fprintf(stdout,"\n@1@tr69_id=%d,beacon=%d,wlan=%d,vlan=%d\n",tr69_id,beacon,wlan_idx,vwlan_idx);
			...
			//fprintf(stdout,"\n@2@tr69_id=%d,beacon=%d,wlan=%d,vlan=%d\n",tr69_id,beacon,wlan_idx,curr_ssid);
			...
			if(wlan_idx==1)
			{
				...
				for(tr69_id=1;tr69_id<=4;tr69_id++)
				{
					if(tr69SetLeafDataInt("Device.WiFi.AccessPoint", tr69_id, "X_ACTIONTEC_COM_BeaconInterval", beacon)) 
					{
						sprintf(error_buf, ("Set Device.WiFi.AccessPoint.%d.X_ACTIONTEC_COM_BeaconInterval error!"),tr69_id);
					}
				}
			}
			if(wlan_idx==0)
